  ...
  switch (event.name) {
    ...
    case balances.dustLost.name:
      if (balances.dustLost.v0.is(event)) {
        const [account, amount] = balances.dustLost.v0.decode(event);
    ...
    case balances.transfer.name:
      if (balances.transfer.v0.is(event)) {
        const [from, to, amount] = balances.transfer.v0.decode(event);
    ...
    case balances.balanceSet.name:
      if (balances.balanceSet.v0.is(event)) {
        ...
        return balances.balanceSet.v9420.decode(event);
      } else {
        throw new UnknownEventVersionError(event.name);
      }
    ...
    case balances.deposit.name:
      if (balances.deposit.v0.is(event)) {
        const [who, amount] = balances.deposit.v0.decode(event);
        return { who, amount };
    ...
    case balances.reserved.name:
      if (balances.reserved.v8.is(event)) {
        const [who, amount] = balances.reserved.v8.decode(event);
        return { who, amount };
    ...
    case balances.unreserved.name:
      if (balances.unreserved.v8.is(event)) {
        const [who, amount] = balances.unreserved.v8.decode(event);
        return { who, amount };
    ...
    case balances.withdraw.name:
      if (balances.withdraw.v9122.is(event)) {
        const [who, amount] = balances.withdraw.v9122.decode(event);
        return { who, amount };
    ...
    case balances.slashed.name:
      if (balances.slashed.v9122.is(event)) {
        const [who, amount] = balances.slashed.v9122.decode(event);
        return { who, amount };
    ...
    case balances.totalIssuanceForced.name:
      if (balances.totalIssuanceForced.v1002000.is(event)) {
        return balances.totalIssuanceForced.v1002000.decode(event);
      } else {
        throw new UnknownEventVersionError(event.name);
      }

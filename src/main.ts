  ...
  async (ctx) => {
    ...
    for (const block of ctx.blocks) {
      const ethereumTransactEventsPerBlock: Map<
        ...
        Map<string, any>
      > = new Map();
      for (const item of orderItems(block)) {
        const pallet = item.value.name.split(".")[0];
        if (item.kind === "event") {
          if (eventNormalizationHandlers[pallet]) {
            if (["EVM.Log"].includes(item.value.name)) {
              switch ((item.value.args.log || item.value.args).topics[0]) {
                case erc20.events.Transfer.topic:
                  try {
                    const parsedEvent = parseEvmLog(item.value);
                    const {
                      from,
                      to,
                      value: amount,
            ...
            if (
              item.value.extrinsic?.call.name === "Ethereum.transact" &&
              item.value.extrinsic?.call.success &&
            ...
            if (
              ...
              const args = eventNormalizationHandlers[pallet](item.value);
              const event = createEventNorm(block.header, item.value, args);
              events.push(event);
              addAddressesToMap(
                item.value.name,
                args,
          ...
          if (callNormalizationHandlers[pallet]) {
            const args = callNormalizationHandlers[pallet](item.call);
...
function createEventNorm(block: Block, event: any, args: any): EventNorm {
...
function createCallNorm(block: Block, call: any, args: any): CallNorm {
  return new CallNorm({
    id: call.id,
    blockHash: block.hash,
    timestamp: block.timestamp ? new Date(block.timestamp) : undefined,
...
function parseEvmLog(event: any): any {
...
function handleEvmTransactions(
  ...
  ethereumTransactEventsPerBlock.forEach((events, extrinsicId) => {
    ...
    const normalizedEthereumTransactionCallArgs =
      callNormalizationHandlers["Ethereum"](ethereumTransactCall);
    const normalizedExtrinsicSuccessEventArgs =
      eventNormalizationHandlers["System"](extSuccessEvent);
...
function orderItems(block: any): any[] {
  const items: any[] = [];

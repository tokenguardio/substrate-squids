...
export const calculationFailed =  {
    name: 'ChannelCommission.CalculationFailed',
    /**
    ...
    v13000: new EventType(
        'ChannelCommission.CalculationFailed',
        sts.struct({
...
export const bifrostCommissionTransferFailed =  {
    name: 'ChannelCommission.BifrostCommissionTransferFailed',
    /**
     * Bifrost commission transfer failed.
     * Parameters are the commission token and the amount that failed to transfer.
     */
    v13000: new EventType(
        'ChannelCommission.BifrostCommissionTransferFailed',
        sts.struct({
            from: v13000.AccountId32,
            to: v13000.AccountId32,
            commissionToken: v13000.CurrencyId,
            amount: sts.bigint(),
        })
    ),
}

export const removalNotCompleteError =  {
    name: 'ChannelCommission.RemovalNotCompleteError',
    /**
     * Error event indicating that the removal process of clearing was not completed.
     */
    v13000: new EventType(
        'ChannelCommission.RemovalNotCompleteError',
        sts.struct({
            targetNum: sts.number(),
            limit: sts.number(),
            executedNum: sts.number(),
        })
    ),
}
